...
# Training and Validation Dataset > china-motorbike_voc
...
# Since for this code, you will access the valid directory of the _voc, point to the valid directory
valid_path = os.path.join(base_dir, 'valid')  

def organize_images_by_defect():
    if not os.path.exists(valid_path):
        print(f'Valid folder not found: {valid_path}')
        return
    ...
    for xml_file in xml_filenames:
        xml_file_path = os.path.join(valid_path, xml_file)
        ...
        for obj in root.iter('object'):
            ...
            defect_folder = os.path.join(valid_path, defect_type)
            os.makedirs(defect_folder, exist_ok=True)

            # Copy the image to the corresponding defect folder
            image_path_to_copy = os.path.join(valid_path, image_name)
